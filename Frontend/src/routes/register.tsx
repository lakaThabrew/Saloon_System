...
import type { Gender } from "@/lib/salon-data";
...
function RegisterPage() {
  ...
  const [form, setForm] = useState({
    ...
    gender: "FEMALE" as Gender, preferences: "",
  });
  ...
  return (
    <SiteLayout>
      <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-16 sm:px-6">
        ...
        <p className="mt-2 text-muted-foreground">Create an account to book and track your visits.</p>
        <form onSubmit={onSubmit} className="mt-8 space-y-4 rounded-2xl border border-border bg-card p-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="preferences">Preferences <span className="text-muted-foreground">(optional)</span></Label>
            <Textarea id="preferences" rows={2} placeholder="Preferred stylist, allergies, etc." value={form.preferences} onChange={(e) => setForm({ ...form, preferences: e.target.value })} />
          </div>
