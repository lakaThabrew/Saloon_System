...
import { SERVICES, STAFF, generateSlotsFor, type TimeSlot } from "@/lib/salon-data";
...
export const Route = createFileRoute("/booking")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reserve your slot online — pick services, stylist and time in seconds." },
    ],
  }),
...
function BookingPage() {
  ...
  const [serviceIds, setServiceIds] = useState<number[]>(preSelected ? [preSelected] : []);
  ...
  useEffect(() => { setSlotId(undefined); }, [staffId, date]);
  ...
  async function confirm() {
    ...
    try {
      ...
      toast.success("Appointment booked", { description: `${appt.serviceNames.join(", ")} · ${appt.date} at ${appt.startTime}` });
      setServiceIds([]); setStaffId(undefined); setSlotId(undefined); setNotes("");
    } catch (err) {
    ...
    <SiteLayout>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 pb-20 sm:px-6 lg:grid-cols-[1fr_360px]">
        <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 text-xs text-muted-foreground">{s.durationMinutes} min · {s.targetGender}</div>
                    </div>
          ...
          {/* Step 2 - Staff */}
          <Step number={2} title="Pick your stylist" complete={!!staffId} disabled={serviceIds.length === 0}>
            {availableStaff.length === 0 ? (
              <p className="text-sm text-muted-foreground">No stylist covers every selected service — please adjust your selection.</p>
            ) : (
                ...
                {availableStaff.map((m) => (
                  <button
                    ...
                    className={cn(
                      ...
                      staffId === m.id ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-primary/40",
                    )}
                  >
                    ...
                    <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{m.experienceYears}+ yrs</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <Step number={4} title="Review & confirm" complete={false} disabled={!canConfirm}>
            {canConfirm && staffId && slotId ? (
              <div className="rounded-xl bg-secondary/50 p-5 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Services</span><span className="text-right">{selectedServices.map((s) => s.serviceName).join(", ")}</span></div>
                <div className="mt-2 flex justify-between"><span className="text-muted-foreground">Stylist</span><span>{STAFF.find((s) => s.id === staffId)!.fullName}</span></div>
                <div className="mt-2 flex justify-between"><span className="text-muted-foreground">When</span><span>{date} · {slots.find((s) => s.id === slotId)?.startTime}</span></div>
                <div className="mt-2 flex justify-between"><span className="text-muted-foreground">Total duration</span><span>{totalMinutes} min</span></div>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">Complete the steps above to see your summary.</p>
            )}
            <div className="mt-4">
              <label className="flex flex-col gap-1 text-sm">
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            {!user ? (
              <p className="mt-2 text-sm text-muted-foreground">
                <Link to="/login" className="underline underline-offset-4">Sign in</Link> to see and manage your bookings.
              </p>
            ) : myAppointments.length === 0 ? (
              <p className="mt-2 text-sm text-muted-foreground">No bookings yet — pick a service to get started.</p>
            ) : (
              <ul className="mt-4 space-y-3">
                {myAppointments.slice().reverse().map((a) => (
                  <li key={a.id} className={cn("rounded-lg border border-border p-3 text-sm", a.status === "CANCELLED" && "opacity-60")}>
                    ...
                    <div className="mt-1 flex items-center justify-between">
                      <span className="text-xs">{a.date} · {a.startTime}</span>
                      {a.status === "BOOKED" ? (
                        <button
                          onClick={() => { cancelAppointment(a.id); toast.message("Booking cancelled"); }}
                          className="text-xs text-destructive hover:underline"
                        >
                          Cancel
                        </button>
                      ) : (
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{a.status}</span>
                      )}
                    </div>
                  </li>
                ))}
...
function Step({ number, title, complete, disabled, children }: {
  number: number; title: string; complete: boolean; disabled?: boolean; children: React.ReactNode;
}) {
  return (
    <section className={cn("rounded-2xl border border-border bg-card p-6 transition-opacity", disabled && "opacity-60")}>
