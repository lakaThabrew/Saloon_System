...
import { CATEGORIES, SERVICES, type ServiceCategory } from "@/lib/salon-data";
...
export const Route = createFileRoute("/services")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Explore our full menu of hair, colour, spa, nails and makeup services." },
    ],
  }),
    ...
    <SiteLayout>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((s) => (
            <article key={s.id} className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-shadow hover:shadow-md">
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <div className="mt-5 flex items-center justify-between border-t border-border pt-4 text-sm">
                  ...
                  <Button size="sm" asChild>
                    <Link to="/booking" search={{ service: s.id }}>Book</Link>
                  </Button>
                </div>
              </div>
            </article>
