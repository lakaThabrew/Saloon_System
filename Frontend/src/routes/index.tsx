    ...
    <SiteLayout>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-10 flex items-center gap-6 text-sm text-muted-foreground">
              <div className="flex items-center gap-1"><Star className="h-4 w-4 fill-[color:var(--gold)] text-[color:var(--gold)]" /><span className="font-medium text-foreground">4.9</span> / 5</div>
      ...
      <section className="border-y border-border/60 bg-secondary/30">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 md:grid-cols-3">
          {[
            { icon: ShieldCheck, title: "Master stylists", body: "10+ years each, trained in Paris & Tokyo." },
            { icon: Clock, title: "On-time, always", body: "We honour your schedule — no waiting rooms." },
            { icon: Sparkles, title: "Clean, calm space", body: "Sanitised tools, single-use linens, soft light." },
          ].map((f) => (
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link to="/services" className="text-sm text-foreground underline underline-offset-4 hover:text-primary">
            See all
          </Link>
        </div>
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {featured.map((s) => (
            <div key={s.id} className="group overflow-hidden rounded-2xl border border-border bg-card transition-shadow hover:shadow-md">
              ...
              <div className="p-5">
                <span className="text-xs uppercase tracking-widest text-muted-foreground">{s.category}</span>
      ...
      <section className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6">
        ...
        <Button size="lg" className="mt-8" asChild>
          <Link to="/booking">Book now <ArrowRight className="ml-1 h-4 w-4" /></Link>
        </Button>
      </section>
    </SiteLayout>
  );
